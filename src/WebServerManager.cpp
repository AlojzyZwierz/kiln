...
String WebServerManager::generateSVG(const Curve &curIn)
{
  ...
  while (i < curveElemsNo)
  {
    ...
    String dashStyle = curveManager.isSkip(i) ? "stroke-dasharray:10,5;" : "";
svg += "<line x1=\"" + String(lastX) + 
       "\" y1=\"" + String(1300 - lastY) + 
       "\" x2=\"" + String(X) + 
       "\" y2=\"" + String(1300 - Y) + 
  ...
  if (MeasurementManager::get().getMeasurements().size() > 0)
  {
